...
async def test_poll_job_status_detects_new_job(mocker, service):
    ...
    mock_hwa_client.return_value.__aenter__.return_value.plan.query_job_streams = AsyncMock(return_value=[
        {'jobStreamName': 'JOB_A', 'status': 'EXEC', 'id': '123', 'workstationName': 'CPU1'}
    ])
...
async def test_poll_job_status_detects_status_change(mocker, service):
    ...
    service.job_cache = {
        'JOB_A': {'jobStreamName': 'JOB_A', 'status': 'PEND', 'id': '123', 'workstationName': 'CPU1'}
    }
...
async def test_handle_status_change_publishes_and_alerts(service):
    ...
    event = JobStatusEvent(
        job_id='123', job_name='JOB_A', old_status='EXEC', new_status='ABEND',
        workstation='CPU1', timestamp=MagicMock()
    )
