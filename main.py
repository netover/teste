...
def main():
    ...
    server_config = uvicorn.Config(app, host=config.SERVER_HOST, port=config.SERVER_PORT, log_level="info")
    ...
    if not os.environ.get("FORCE_CONSOLE_MODE") == "1":
        try:
            ...
            logging.warning(f"Could not import GUI libraries, falling back to console mode. Error: {e}")
            pystray_available = False
