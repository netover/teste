...
from src.services.ml.models import WorkstationForecastResponse, WorkloadForecast, ForecastDatapoint
...
class WorkloadForecaster:
    ...
    async def train_workload_forecast(self, historical_data: pd.DataFrame):
        ...
        for workstation in historical_data['workstation'].unique():
            ws_data = historical_data[historical_data['workstation'] == workstation].copy()
    ...
    async def forecast_workload(self, workstation: str, days_ahead: int = 7) -> WorkstationForecastResponse:
        ...
        """
        logging.info(f"Generating workload forecast for workstation '{workstation}' for {days_ahead} days.")
        ...
        for metric in ['job_count', 'total_runtime', 'cpu_usage']:
            ...
            if model_key in self.models:
                ...
                forecasts[metric] = WorkloadForecast(
                    ...
                    trend="increasing" if forecast_df['trend'].iloc[-1] > forecast_df['trend'].iloc[-days_ahead-1] else "decreasing",
                    seasonality_strength=float(forecast_df[['weekly', 'yearly']].abs().mean().mean())
                )

        if not forecasts:
            raise ValueError(f"No models found for workstation '{workstation}'. Please train the models first.")
