...
def get_asset_urls(entry_point: str) -> dict:
    ...
    In production (and testing), it reads the manifest file.
    """
    # In development mode, Vite serves the files directly with HMR.
    ...
    # In production or testing, we use the pre-built assets from the manifest.
    ...
    if not _manifest_cache or f"static/js/{entry_point}" not in _manifest_cache:
        raise RuntimeError(f"Entry point '{entry_point}' not found in manifest.json. Run 'npm run build'.")
